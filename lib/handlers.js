...
handlers.index = function (data, callback) {
    ...
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'We offer free, simple uptime monitoring for HTTP/HTTPS sites of all kinds. When your site goes down, we\'ll send you a text to notice you',
            'body.class': 'index'
        };
                        ...
                        if (!err && userData) {
                            ...
                            _data.update('users', phone, userData, function (err) {
                                if (!err) {
                                    ...
                                    console.log(err);
                                    callback(500, { 'Error': 'Could not update the user' }); // Internal error: 500
                                }
                            });
                        } else {
...
handlers._checks.post = function (data, callback) {
    ...
    if (protocol && url && method && successCodes && timeoutSeconds) {
        // Check the user has provided a token
        var token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
        ...
        _data.read('tokens', token, function (err, tokenData) {
            if (!err && tokenData) {
                ...
                callback(403); //Not authorized user
            }
        });
    } else {
    ...
    if (id) {
        ...
        _data.read('checks', id, function (err, checkData) {
            if (!err) {
                ...
                // Verify that specified token is valid and bleongs to the user who created the checks
                                ...
                                _data.read('users', checkData.userPhone, function (err, userData) {
                                    if (!err) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, { 'Error': 'Could not find the check on the user\'s object, so cannot delete it' });
                                        }
