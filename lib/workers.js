...
workers.performCheck = function (originalCheckData) {
    ...
    // parse the hotname and the path out of the original check data
...
// Process the check outcome, update the check data as needed, then trigger an alert to the user
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function (e) {
        if (!e) {
...
workers.log = function (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) {
    ...
    // Append the log tring to the file
        ...
        if (!err && logs && logs.length > 0) {
            logs.forEach(function (logName) {
                ...
                _logs.compress(logId, newFileId, function (err) {
                    if (!err) {
                        // Truncate the log (Empty the log file after moving all data out of it)
...
// Timer to execute the log-rotation process once per dady
...
workers.init = function () {
    ...
    // Call the loop so the checks will execute automatically later on
    workers.loop();
